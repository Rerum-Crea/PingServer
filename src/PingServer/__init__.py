...
class launch_pages():
    def __init__(self):
        from threading import Thread
        thread_data = Thread(target=launch_pages.launch_pages_internals())
        return thread_data
...
def initialize(amt=10, done=False, html=False):
    if not done:
...
def initialize_all(amt=10):
    initialize(amt, False, False)
    initialize(amt, False, True)
